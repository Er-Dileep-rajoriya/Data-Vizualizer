    ...
    if (query && isQueryPresent(query)) {
      if (loadSql === 'only') {
        cubeApi.sql(query, {
          ...
          error: null,
          isLoading: false
        })).catch(error => this.setState({ ...(resetResultSetOnChange ? {
            resultSet: null
          } : {}),
          error,
          isLoading: false
...
const QueryRendererWithTotals = ({
  ...
  queries: {
    totals: { ...query,
      dimensions: [],
      timeDimensions: query.timeDimensions ? query.timeDimensions.map(td => ({ ...td,
        granularity: null
      })) : undefined
    },
    main: query
...
class QueryBuilder extends React.Component {
  ...
  static resolveMember(type, {
    ...
    if (type === 'timeDimensions') {
      return (query.timeDimensions || []).map((m, index) => ({ ...m,
        dimension: { ...meta.resolveMember(m.dimension, 'dimensions'),
          granularities
        },
        index
      }));
  ...
  cubeApi() {
    const {
      cubeApi
    } = this.props; // eslint-disable-next-line react/destructuring-assignment
  ...
  prepareRenderProps(queryRendererProps) {
    ...
    const flatFilters = uniqBy(filter => `${prop('member', filter)}${prop('operator', filter)}`, flattenFilters(meta && query.filters || []).map(filter => ({ ...filter,
      member: filter.member || filter.dimension
    })));
    const filters = flatFilters.map((m, i) => ({ ...m,
    ...
    let orderMembers = uniqBy(prop('id'), [// uniqBy prefers first, so these will only be added if not already in the query
    ...members.map(({
      name,
      title
    }) => ({
    ...
    return {
      ...
      updateOrder: {
        set: (memberId, newOrder = 'asc') => {
          this.updateQuery({
            order: orderMembers.map(orderMember => ({ ...orderMember,
  ...
  async updateVizState(state) {
    ...
    const handleVizStateChange = currentState => {
      ...
      if (onVizStateChanged) {
        const newVizState = pick(['chartType', 'pivotConfig', 'query'], currentState); // Don't run callbacks more than once unless the viz state has changed since last time
    ...
    if (shouldFetchDryRun && isQueryPresent(finalState.query) && finalState.missingMembers.length === 0) {
      try {
        ...
        if (finalState.shouldApplyHeuristicOrder) {
          finalState.query.order = (response.queryOrder || []).reduce((memo, current) => ({ ...memo,
            ...current
          }), {});
        }
...
function useCubeSql(query, options = {}) {
  return useCubeFetch('sql', { ...options,
    query
  });
}

function useDryRun(query, options = {}) {
  return useCubeFetch('dryRun', { ...options,
    query
  });
}
